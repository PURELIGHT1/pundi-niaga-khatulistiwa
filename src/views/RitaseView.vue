<template>
  <VFadeTransition style="transition-duration: var(--transition-duration) !important">
    <VLayout v-if="appStore.active.child" class="d-scroll pa-2 overflow-auto" width="0px" min-height="fit-content"
      max-height="100vh">
      <VLayout class="d-flex flex-column ma-auto" min-width="fit-content">
        <VForm class="mb-2 d-flex">
          ...
          <VSlideXTransition group>
            <template v-if="ritaseStore.inputYear">
  ...
  watch(
    ...
    (values) => {
      ...
      if (window.Worker) {
        ...
        ritaseStore.worker = new Worker(
          new URL('@/workers/ritase-worker.ts', import.meta.url),
          { type: 'module' }
        );
