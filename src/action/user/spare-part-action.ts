...
export function sparePartActions() {
  ...
  const createSparePart = async (
    ...
    sparePart.push({
      ...
      butuh_konfirmasi:
        appStore.user.jenis === UserJenis.Pemilik ? true : false,
    });
    ...
    appStore.setAlerts(
      "success",
      "Aksi Berhasil",
      "Berhasil Tambah Data Spare Part",
    );
  };
  ...
  const updateSparePart = async (
    ...
    appStore.setAlerts(
      "success",
      "Aksi Berhasil",
      "Berhasil Ubah Data Spare Part",
    );
  };

  const updateStatusSparePart = async (
    form: SparePartForm,
    extraData?: ActionModel,
  ) => {
    if (appStore.loading) {
    ...
    appStore.setAlerts(
      "success",
      "Aksi Berhasil",
      "Berhasil Ubah Status Spare Part",
    );
  };
  ...
  const konfirmasiSparePart = async (form: SparePartForm) => {
    ...
    if (index !== -1) {
      ...
      const pemilik = userStore.users.find(
        (user) => user.jenis === UserJenis.Pemilik,
      );
  ...
  const deleteSparePart = async (form: SparePartForm) => {
    ...
    appStore.setAlerts(
      "success",
      "Aksi Berhasil",
      "Berhasil Hapus Data Spare Part",
    );
  };
