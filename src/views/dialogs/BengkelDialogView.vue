<template>
  <VLayout class="position-relative mb-8">
    ...
    <span class="text-h6 font-weight-bold text-center mx-auto"> {{ getTitle }} Bengkel </span>
  </VLayout>
  ...
  <template v-else>
    <VForm>
      <CustomTextField
        v-model="userStore.form.nama_lengkap.value"
        label="Nama Lengkap"
        ...
        autocomplete="name"
        @update:model-value="(val) => onFieldChange(userFieldForm.nama_lengkap, val)"
      />
      <VDivider class="my-1" />

      <CustomTextField
        v-model="userStore.form.nomor_hp.value"
        label="Nomor HP"
        ...
        prefix="+62"
        @update:model-value="(val) => onFieldChange(userFieldForm.nomor_hp, val)"
      />
      <VDivider class="my-1" />

      <CustomTextField
        v-model="userStore.form.email.value"
        label="Email"
        ...
        autocomplete="email"
        @update:model-value="(val) => onFieldChange(userFieldForm.email, val)"
      />
      <VDivider class="my-1" />
    </VForm>
  </template>
  ...
  <VBtn
    ...
    @click="
    () => {
      (userStore as any)[action](UserJenis.Bengkel, props);
    }
  "
  >
    {{ getTitle }}
