<template>
  ...
  <template v-else>
    <VForm>
      <CustomSelect v-model="pengeluaranStore.form.tipe.value" :items="Object.values(PengeluaranType)"
        label="Tipe Pengeluaran" placeholder="Pilih Salah Satu"
      ...
      <VSlideYTransition>
        <template v-if="pengeluaranStore.form.tipe.value === PengeluaranType.Mobil">
          <CustomSelect v-model="pengeluaranStore.form.mobil.value" :items="mobilStore.mobils.sort((a, b) =>
            a.nomor_polisi.toLowerCase().localeCompare(b.nomor_polisi.toLowerCase()),
          )
            " item-value="id" item-title="nomor_polisi" :item-props="(item: Mobil) => ({
      ...
      <CustomTextArea v-model="pengeluaranStore.form.keterangan.value" label="Keterangan"
        placeholder="Maksimal 500 Karakter" :prepend-inner-icon="{
          active: '$text-box',
          passive: '$text-box-outline',
        }" clearable counter @update:model-value="(value) => onFieldChange(pengeluaranFieldForm.keterangan, value)" />
      <VDivider class="my-1" />

      <CustomTextField v-model="pengeluaranStore.form.biaya.value" label="Biaya" placeholder="1.000.000"
        :prepend-inner-icon="{
          active: '$cash-100',
          passive: '$cash-minus',
        }" prefix="Rp" @update:model-value="(value) => onFieldChange(pengeluaranFieldForm.biaya, value)" />
      <VDivider class="my-1" />

      <CustomDatePicker v-model="pengeluaranStore.form.tanggal.value" title="Tanggal" :max="new Date()"
