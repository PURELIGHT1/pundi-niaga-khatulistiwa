...
export async function checkSparePartValidation(
  ...
  const [merk, jenis, nama, harga, stok, status_pembelian] = await Promise.all([
    action !== 'updateStatus' ? merekValidation(form.merek.value) : { error: '' },
    action !== 'updateStatus' ? jenisValidation(form.jenis.value) : { error: '' },
    action !== 'updateStatus' ? namaValidation(form.nama.value) : { error: '' },
    action !== 'updateStatus' ? hargaValidation(form.harga.value) : { error: '' },
    action !== 'updateStatus' ? stokValidation(form.stok.value) : { error: '' },
    action !== 'update' ? statusValidation(form.status_pembelian.value) : { error: '' },
  ])
