...
export async function namaValidation(nama: string, id: number | null): Promise<{ value: string; error: string }> {
  ...
  if (!value) {
    ...
    const pks = pksStore.pks.find((pks) => pks.nama.toLowerCase() === value.toLowerCase());
    if (pks !== null &&  pks.id !== id) {
