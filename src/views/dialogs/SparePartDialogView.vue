<template>
  ...
  <template v-else-if="props.action === 'updateStatus'">
    ...
    <CustomSelect v-model="sparePartStore.form.status_pembelian.value" :items="Object.values(StatusSparePart)"
      label="Status Pembelian Spare Part" placeholder="Pilih Salah Satu" @update:mgasodel-value="
        (val: StatusSparePart) =>
  ...
  <template v-else>
    <VForm>
      <CustomTextField v-model="sparePartStore.form.merek.value" label="Merek" placeholder="Maksimal 50 Karakter"
        :prepend-inner-icon="{
          active: '$cog',
          passive: '$cog-outline',
      ...
      <CustomTextField v-model="sparePartStore.form.jenis.value" label="Jenis" placeholder="Maksimal 50 Karakter"
        :prepend-inner-icon="{
          active: '$wrench-cog',
      ...
      <CustomTextField v-model="sparePartStore.form.nama.value" label="Nama" placeholder="Maksimal 50 Karakter"
        :prepend-inner-icon="{
      ...
      <CustomTextField v-model="sparePartStore.form.harga.value" label="Harga" placeholder="1.000.000"
        :prepend-inner-icon="{
          active: '$tag',
          passive: '$tag-outline',
        }" prefix="Rp" @update:model-value="(val) => onFieldChange(sparePartFieldForm.harga, val)" />
      <VDivider class="my-1" />

      <CustomTextField v-model="sparePartStore.form.stok.value" label="Stok" placeholder="100" :prepend-inner-icon="{
        active: '$clipboard-list',
        passive: '$clipboard-list-outline',
      }" @update:model-value="(val) => onFieldChange(sparePartFieldForm.stok, val)" />
      <VDivider class="my-1" />

      <CustomSelect v-if="appStore.user.jenis === UserJenis.Pemilik && props.action == UserAction.Create"
        v-model="sparePartStore.form.status_pembelian.value" :items="Object.values(StatusSparePart)"
        label="Status Pembelian Spare Part" placeholder="Pilih Salah Satu" :prepend-inner-icon="{
...
<script setup lang="ts">
  ...
  import { type FormattedSparePart, StatusSparePart } from '@/models/spare-part-model'
