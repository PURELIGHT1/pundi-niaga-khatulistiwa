...
export function userActions() {
  ...
  const login = async (form: UserForm) => {
    ...
    const findUser = users.find((u) => u.email === email && u.kata_sandi === kata_sandi)

    if (!findUser) {
      appStore.loading = false
      appStore.setAlerts('error', 'Autentikasi Gagal', 'Email atau Password Salah')
      return
    }
    ...
    appStore.setAlerts('success', 'Autentikasi Berhasil', `Hai ${findUser.nama_lengkap} 👋`)
  }
  ...
  const logout = async () => {
    ...
    appStore.setAlerts('success', 'Keluar Berhasil', `Sampai Jumpa ${user.nama_lengkap} 👋`)

    resetUser()
    router.push({ name: 'Masuk' })
  }

  const createUser = async (jenis: UserJenis, form: UserForm, extraData?: ActionModel) => {
    if (appStore.loading) {
      return
    }
    ...
    afterAction(form)
    appStore.setAlerts('success', 'Aksi Berhasil', `Berhasil Tambah Data ${jenis}`)
  }
  ...
  const updateUser = async (form: UserForm, extraData?: ActionModel) => {
    ...
    afterAction(form)
    appStore.setAlerts('success', 'Aksi Berhasil', `Berhasil Ubah Data ${item.jenis}`)
  }
  ...
  const deleteUser = async (form: UserForm) => {
    ...
    afterAction(form)
    appStore.setAlerts('success', 'Aksi Berhasil', `Berhasil Hapus Data ${item.jenis}`)
  }
