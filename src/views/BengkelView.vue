          ...
          <template #top>
            <VLayout class="pt-1 pb-2">
              <VBtn
                height="56px"
                color="secondary"
                @click="appStore.setDialog(undefined, BengkelDialogView, 'create')"
              >
                Tambah Bengkel
              </VBtn>
          ...
          <template #item.opsi="{ item }">
            <VLayout>
              <VBtn
                ...
                :elevation="0"
                color="secondary"
                @click="appStore.setDialog(item, BengkelDialogView, 'update')"
              >
              ...
              <VBtn
                ...
                :elevation="0"
                color="error"
                @click="appStore.setDialog(item, BengkelDialogView, 'delete')"
              >
...
import BengkelDialogView from './dialogs/BengkelDialogView.vue'
...
watch(
  ...
  (values) => {
    if (values[0] && values[1]) {
      userStore.formattedUsers = getFormattedUsers(userStore.users, UserJenis.Bengkel)
