<template>
  ...
  <template v-else>
    <VForm>
      <CustomSelect v-model="pendapatanStore.form.sopir.value" :items="sopirStore.sopirs.sort((a, b) =>
        a.nama_lengkap.toLowerCase().localeCompare(b.nama_lengkap.toLowerCase()),
      )
        " item-value="id" item-title="nama_lengkap" :item-props="(item: Sopir) => ({
          title: item.nama_lengkap,
          subtitle: item.nomor_hp,
        })
          " label="Sopir" placeholder="Pilih Salah Satu"
        :prepend-inner-icon="{ active: '$ship-wheel', passive: '$steering' }" return-object
      ...
      <CustomSelect v-model="pendapatanStore.form.mobil.value" :items="mobilStore.mobils.sort((a, b) =>
        a.nomor_polisi.toLowerCase().localeCompare(b.nomor_polisi.toLowerCase()),
      )
        " item-value="id" item-title="nomor_polisi" :item-props="(item: Mobil) => ({ title: item.nomor_polisi, subtitle: item.nama })
          " label="Mobil" placeholder="Pilih Salah Satu"
        :prepend-inner-icon="{ active: '$car', passive: '$car-outline' }" return-object
      ...
      <CustomTextField v-model="pendapatanStore.form.spk.value" label="SPK" placeholder="Maksimal 100 Karakter"
        :prepend-inner-icon="{ active: '$barcode-scan', passive: '$barcode' }"
        @update:model-value="(val) => onFieldChange(pendapatanFieldForm.spk, val)" />
      <VDivider class="my-1" />

      <CustomDatePicker v-model="pendapatanStore.form.tanggal_muat.value" title="Tanggal Muat" :max="new Date()"
      ...
      <VSlideYTransition group>
        <template v-if="pendapatanStore.form.tanggal_muat.value">
          <CustomDatePicker v-model="pendapatanStore.form.tanggal_bongkar.value" title="Tanggal Bongkar"
            :min="new Date(pendapatanStore.form.tanggal_muat.value)" :max="new Date()" @update:model-value="
      ...
      <CustomTextField v-model="pendapatanStore.form.timbangan_muat.value" label="Timbangan Muat" placeholder="1.000"
        :prepend-inner-icon="{
          active: '$scale-unbalance',
          passive: '$scale-balance',
        }" suffix="Kg" @update:model-value="(val) => onFieldChange(pendapatanFieldForm.timbang_muat, val)" />
      <VDivider class="my-1" />

      <CustomTextField v-model="pendapatanStore.form.timbangan_bongkar.value" label="Timbangan Bongkar"
        placeholder="1.000" :prepend-inner-icon="{
          active: '$scale-unbalance',
          passive: '$scale-balance',
        }" suffix="Kg" @update:model-value="(val) => onFieldChange(pendapatanFieldForm.timbang_bongkar, val)" />
      <VDivider class="my-1" />

      <CustomTextField v-model="pendapatanStore.form.ongkos_angkut.value" label="Ongkos Angkut" placeholder="100"
        :prepend-inner-icon="{ active: '$truck', passive: '$truck-outline' }" prefix="Rp" suffix="/Kg"
        @update:model-value="(val) => onFieldChange(pendapatanFieldForm.ongkos_angkut, val)" />
      <VDivider class="my-1" />

      <CustomTextField v-model="pendapatanStore.form.harga_klaim_susut.value" label="Harga Klaim Susut"
        placeholder="10.000" :prepend-inner-icon="{ active: '$cash-100', passive: '$cash' }" prefix="Rp"
...
<script setup lang="ts">
  ...
  import type { FormattedPendapatan } from '@/models/pendapatan-model'
