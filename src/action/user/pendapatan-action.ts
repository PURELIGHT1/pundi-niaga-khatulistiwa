...
export function pendapatanActions() {
  ...
  const createPendapatan = async (
    ...
    pendapatans.push({
      id:
        pendapatans.length === 0
          ? 1
          : pendapatans[pendapatans.length - 1].id + 1,
      id_sopir: sopir.id,
      id_mobil: mobil.id,
      spk: spk,
    ...
    appStore.setAlerts(
      "success",
      "Aksi Berhasil",
      "Berhasil Tambah Data Pendapatan",
    );
  };

  const updatePendapatan = async (
    form: PendapatanForm,
    extraData?: ActionModel,
  ) => {
    if (appStore.loading) {
    ...
    const index = pendapatans.findIndex(
      (pendapatan) => pendapatan.id === item.id,
    );
    if (index !== -1) {
  ...
  const deletePendapatan = async (
    ...
    const item = appStore.dialog.item as FormattedPendapatan;
    appStore.loading = true;
    const response = await checkDeletePendapatanValidation(
      extraData?.action ?? "",
    );
    if (!response.value) {
      appStore.loading = false;
    ...
    appStore.setAlerts(
      "success",
      "Aksi Berhasil",
      "Berhasil Hapus Data Pendapatan",
    );
  };
